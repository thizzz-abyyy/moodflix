...
import { fetchMoodRows, fetchHiddenGems } from '../services/tmdb';
...
const MOOD_EXPLANATION = {
  ...
  angry:    'Intense action and adrenaline-pumping blockbusters to channel that energy.',
...
export function RecsPage({ mood, movies, loading, userData, onOpenMovie, onApplyFilters }) {
  const [filters, setFilters] = useState({ rating: '', platform: '', language: '', year: '', query: '' });
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => { onApplyFilters(mood, filters); }, 500);
      ...
      <div className="recs-header">
        <div className="recs-title-row">
          <h1 className="recs-h1 gradient-text">
            {mood ? `${emojiMap[mood] || '🎬'} ${moodLabel} Picks` : 'Your Recommendations'}
          </h1>
          {mood && <span className="mood-badge">{emojiMap[mood]} {mood}</span>}
        </div>
        ...
        <div className="ai-explanation-banner glass">
          ...
          <div className="ai-exp-content">
            ...
            <div className="ai-exp-text">
              Based on your <strong style={{ color:'var(--mood-primary)' }}>{mood}</strong> mood, our AI selected a mix of English & Tamil films that best match your emotional state.
              We balance trending hits with hidden gems and classics for a perfect watchlist.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="search-filter-container glass">
        <div className="search-bar-wrap">
          ...
          <input
            ...
            value={filters.query}
            onChange={e => { updateFilter('query', e.target.value); setViewMode('grid'); }}
          />
        </div>
        <div className="filter-bar">
          {[
            { label:'⭐ Rating', key:'rating', opts:[['','Any Rating'],['6','6+'],['7','7+'],['8','8+'],['9','9+']] },
            { label:'📺 OTT', key:'platform', opts:[['','Any Platform'],['8','Netflix'],['119','Prime Video'],['122','Disney+ Hotstar'],['232','Zee5'],['237','SonyLIV']] },
            { label:'🌐 Language', key:'language', opts:[['','Any Language'],['en','English'],['ta','Tamil'],['hi','Hindi'],['te','Telugu'],['ml','Malayalam']] },
            { label:'📅 Year', key:'year', opts:[['','Any Year'],['2023','2023+'],['2020','2020+'],['2015','2015+'],['2010','2010+']] },
          ].map(({ label, key, opts }) => (
            <div key={key} style={{ position:'relative', flex:'1 1 150px' }}>
              <select
                ...
                value={filters[key]}
                onChange={e => { updateFilter(key, e.target.value); setViewMode('grid'); }}
                aria-label={label}
              >
        ...
        <>
          ...
          <div className="section" style={{ position:'relative' }}>
            <div className="section-header">
              <div>
                ...
                <p className="section-sub">50% English · 50% Tamil · Curated by AI</p>
              </div>
            </div>
            {rowsLoading
              ? <SkeletonGrid count={6} />
              : <Carousel movies={moodRows?.topPicks || []} mood={mood} userData={userData} onOpen={onOpenMovie} />
            }
          </div>
          ...
          <div className="section" style={{ position:'relative' }}>
            <div className="section-header">
              <div>
                ...
                <p className="section-sub">What everyone is watching this week</p>
              </div>
            </div>
            {rowsLoading
              ? <SkeletonGrid count={6} />
              : <Carousel movies={moodRows?.trending || []} mood={mood} userData={userData} onOpen={onOpenMovie} />
            }
          </div>
          ...
          {userData?.history?.length > 0 && (
            <div className="section" style={{ position:'relative' }}>
              ...
              {rowsLoading
                ? <SkeletonGrid count={6} />
                : <Carousel movies={moodRows?.topPicks?.slice(5, 15) || []} mood={mood} userData={userData} onOpen={onOpenMovie} />
              }
            </div>
          )}
          ...
          <div className="section" style={{ position:'relative' }}>
            <div className="section-header">
              <div>
                ...
                <p className="section-sub">Underrated films with outstanding ratings</p>
              </div>
            </div>
            {rowsLoading
              ? <SkeletonGrid count={6} />
              : <Carousel movies={hiddenGems} mood={mood} userData={userData} onOpen={onOpenMovie} />
            }
          </div>
        </>
      )}
      ...
      {viewMode === 'grid' && (
        <div className="section">
          <div className="section-header">
            ...
            <span style={{ fontSize:'0.85rem', color:'rgba(255,255,255,0.4)' }}>{movies.length} movies</span>
          </div>
          <MovieGrid movies={movies} mood={mood} userData={userData} onOpen={onOpenMovie} loading={loading} />
        </div>
      )}
    </div>
  );

...
export const OTT_MAP = {
  ...
  119: { name: 'Prime Video',     color: '#00A8E0', bg: 'rgba(0,168,224,.2)',  icon: '📦' },
  122: { name: 'Disney+ Hotstar', color: '#1F80E0', bg: 'rgba(31,128,224,.2)', icon: '⭐' },
  232: { name: 'Zee5',            color: '#7B2FD9', bg: 'rgba(123,47,217,.2)', icon: '🟣' },
  237: { name: 'SonyLIV',         color: '#E51C23', bg: 'rgba(229,28,35,.2)',  icon: '📺' },
...
export const GENRE_NAMES = {
  ...
  36: 'History', 10752: 'War', 37: 'Western',
};

const LANG_LABELS = { en: 'EN', ta: 'TA', hi: 'HI', te: 'TE', ml: 'ML', ko: 'KO', ja: 'JA', fr: 'FR', es: 'ES' };
...
export function normalizeMovie(m, otts = []) {
  const genreList = (m.genre_ids || []).map(id => GENRE_NAMES[id]).filter(Boolean).slice(0, 3).join(', ');
  let ott = null;
  for (const p of otts) if (OTT_MAP[p.provider_id]) { ott = { ...OTT_MAP[p.provider_id], id: p.provider_id }; break; }
  ...
  return {
    ...
    language: lang,
    langLabel: LANG_LABELS[lang] || lang.toUpperCase(),
    isTamil: lang === 'ta',
...
export async function analyzeMoodMultiModal({ text, emojiMood, sliderValue, faceEmotion, voiceMood }) {
  const res = await fetch('/api/mood', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ text, emojiMood, sliderValue, faceEmotion, voiceMood }),
  });

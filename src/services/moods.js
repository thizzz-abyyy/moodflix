...
export const MOODS_LIST = Object.keys(MOOD_EMOJI)
  .filter((m) => m !== "neutral")
  .map((id) => ({
    id,
...
export const TIME_NOTES = {
  morning: {
    emoji: "☀️",
    text: "Good morning! Light, feel-good movies recommended.",
  },
  afternoon: {
    emoji: "🌤️",
    text: "Good afternoon! Perfect time for casual entertainment.",
  },
  evening: {
    emoji: "🌆",
    text: "Good evening! Romantic or thrilling picks for you.",
  },
  night: {
    emoji: "🌙",
    text: "Late night mode — deep thrillers and emotional dramas.",
  },
};
...
export function analyzeMood(text) {
  ...
  for (const [mood, kws] of Object.entries(MOOD_KEYWORDS)) {
    scores[mood] = kws.reduce(
      (s, kw) => s + (t.includes(kw) ? (kw.includes(" ") ? 3 : 1) : 0),
      0,
    );
  }
